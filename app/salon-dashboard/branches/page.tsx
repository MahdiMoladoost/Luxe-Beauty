...
import { BranchManager } from "@/components/provider-panel/branch-manager"
    ...
    return (
      <section className="mx-auto flex min-h-[65vh] max-w-2xl items-center justify-center">
        <div className="w-full rounded-[30px] border border-[#5b4033]/10 bg-white p-9 text-center shadow-[0_24px_70px_rgba(65,42,31,0.08)]">
          ...
          <h2 className="mt-5 text-2xl font-black text-[#34231d]">مجموعه‌ای برای مدیریت شعب وجود ندارد</h2>
    ...
    <div className="space-y-6">
      <section className="rounded-[28px] border border-[#5b4033]/10 bg-white p-5 shadow-[0_18px_55px_rgba(66,43,32,0.06)] sm:p-7">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-[#34231d]">مدیریت شعب</h2>
            <p className="mt-2 max-w-3xl text-sm leading-7 text-[#79685f]">
              شعب {provider.nameFa} را ایجاد، ویرایش، فعال یا حذف کنید. تمام تغییرات با کنترل مالکیت، جغرافیا و نسخه هم‌زمان انجام می‌شوند.
            </p>
          </div>
        </div>
      </section>

      <BranchManager key={provider.id} providerId={provider.id} />
    </div>
  )
}
